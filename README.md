...
| Method | URL | 설명 | 인증 |
|--------|-----|------|------|
| POST | `/attachments` | 게시글 저장 전에 첨부파일을 사전 업로드하고 attachmentId 반환 | USER |
...
게시글에 연결될 첨부파일 메타데이터를 관리하는 엔티티입니다.  
파일 본문은 DB에 저장하지 않고 S3 호환 스토리지에 저장하며, DB에는 접근 URL과 파일 메타데이터만 저장합니다.

| 필드 | 타입 | 설명 |
|------|------|------|
| id | Long | 첨부파일 ID |
| post | Post | 연결된 게시글, 사전 업로드 상태에서는 null |
| storeUrl | String | 스토리지에 저장된 파일 접근 URL |
| originalFileName | String | 사용자가 업로드한 원본 파일명 |
| contentType | String | 파일 MIME 타입 |
| status | AttachmentStatus | TEMP / CONFIRMED 상태 |
| createdAt | LocalDateTime | 업로드 시각 |
| fileSize | Long | 파일 크기(byte) |
...
- 첨부파일은 게시글 저장 전에 먼저 업로드합니다.
- 업로드된 파일은 S3 호환 스토리지에 저장하고, DB에는 파일 URL과 메타데이터만 저장합니다.
- 게시글 생성 시 전달된 attachmentIds를 기준으로 첨부파일을 게시글에 연결합니다.
- 게시판 정책에 따라 첨부파일 필수 여부와 최대 첨부 개수를 검증합니다.
- 게시글에 연결되지 않은 TEMP 상태 첨부파일은 스케줄러를 통해 정리합니다.
