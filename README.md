...
Web : https://coreboard-web.vercel.app/
...
- 조회수 증가 정책 구현 및 중복 조회 처리 기준 적용
- 배포 환경에서 S3 호환 스토리지를 R2 등 외부 스토리지로 전환
- 검색 조건이 많아질 경우 검색 전용 전략 분리 검토
- 운영 로그와 지표를 바탕으로 느린 API와 느린 쿼리 확인
- 프론트엔드와 연결해 실제 사용 흐름 검증
